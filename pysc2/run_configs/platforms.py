...
class LocalBase(lib.RunConfig):
  ...
  def __init__(self, base_dir, exec_name, version, cwd=None, env=None):
    ...
    # _exec_name is set above so scan-for-binary helpers work during RunConfig init.
  ...
  def start(self, want_rgb=True, **kwargs):
    ...
    if not os.path.exists(exec_path) and self.version.game_version == "latest":
      # Blizzard may leave higher-numbered Base***** folders behind after failed
      # or partial updates without SC2_x64.exe; naive max(build) points there.
      build = self._newest_installed_build_with_executable()
      if build is not None:
        exec_path = os.path.join(
            self.data_dir, "Versions/Base%05d" % build, self._exec_name)
        self.version = self.version._replace(build_version=build)
  ...
  def get_versions(self, containing=None):
    ...
    # Prefer the newest Base***** that contains the launcher; max dirname alone can
    # point at leftover empty folders after a partial Blizzard update.
    latest_build = self._newest_installed_build_with_executable()
    if latest_build is None:
      latest_build = max(versions_found)
    known_versions.append(
        lib.Version("latest", latest_build, None, None))
  ...
  def _newest_installed_build_with_executable(self):
    """Highest Base***** under Versions that contains self._exec_name (e.g. SC2_x64.exe)."""
    versions_dir = os.path.join(self.data_dir, "Versions")
    version_prefix = "Base"
    try:
      ...
      return None
    builds = sorted(
        (int(v[len(version_prefix):]) for v in names
         if v.startswith(version_prefix)), reverse=True)
    for bv in builds:
      p = os.path.join(
          self.data_dir, "Versions/Base%05d" % bv, self._exec_name)
      if os.path.isfile(p):
        return bv
    return None
